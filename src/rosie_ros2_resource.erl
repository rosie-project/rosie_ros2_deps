...
repo_description_contains_pkg(Pkg,
        ...
        [] ->
            case [R || R <- REPOS, repo_matches_pkg(Pkg, R)] of
                [R] ->
                    get_source_url_from_repo_tuple(R);
                [] ->
                    rebar_api:warn("Package ~p could not be found in the the downloaded official distribution.yaml",
                                   [Pkg]),
                    not_found
            end
    end.
...
add_deps(Dir, AppName, CustomState) ->
    ...
    PkgDependencies =
        lists:flatten([X#xmlElement.content || X <- Content, X#xmlElement.name == depend]),
...
convert_repo_to_rebar3_project(Dir, AppInfo, CustomState) ->
  ...
  {application, "
                    ++ AppName
                    ++ ",
   [{description, \"Generated app from ros2 pkg: "
                    ++ AppName
                    ++ "\"},
    {vsn, \"0.0.0\"},
    {registered, []},
